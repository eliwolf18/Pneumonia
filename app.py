...
import warnings
warnings.simplefilter(action='ignore', category=DeprecationWarning) # To ignore warnings

# Loading the trained model from jupyter
...
# Converting the legend data to a DataFrame for display
...
# Model accuracy metrics from trained model
model_metrics = {
...
- **LSTM**: 0.6654  
  Low accuaracy due to small dataset used. Neural networks works well with very large amount of data.
    

**Interpretation**:  
With an R² of 96.86% and extremely low error metrics, the model demonstrates exceptional predictive accuracy.
"""
# Application layout using html and css for designs
app.layout = html.Div(
    ...
    children=[
        ...
        html.Button('Get Prediction', id='predict-button', n_clicks=0, style={'width': '220px', 'margin': '20px auto', 'display': 'block', 'fontWeight': 'bold', 'backgroundColor': '#007BFF', 'color': 'white', 'border': 'none', 'padding': '10px', 'borderRadius': '5px'}), 

        # Output for the predicted text
...
def predict_recovery_time(n_clicks, scatter_feature, age, duration, signs, temperature, respiratory_rate, time,
    ...
    if n_clicks > 0:
        # Check if any feature is empty or zero
        if not all([age, duration, signs, temperature, respiratory_rate, time, clinical_score, km, hazard_function]):
            error_message = "Error: Features cannot be empty or zero. Please fill in all the fields."
            return error_message, go.Figure(), 0, go.Figure(), go.Figure()
        ...
        input_data = pd.DataFrame([[
            ...
            'age', 'duration', 'signs', 'temperature', 'respiratory rate', 'time',
            'clinical score', 'kaplan meier survival estimate', 'Hazard function',
            'drug administered_cristaline_pencillin', 'drug administered_ceftriaxon',
            'drug administered_ampicillin&gentamicin'
        ])
        ...
        recovery_times = [model.predict(pd.DataFrame([[age if scatter_feature != 'age' else val,
                                                       ...
                                                       hazard_function,
                                                       drug_pencillin, drug_ceftriaxon, drug_ampicillin]],
                                                     columns=input_data.columns))[0] for val in feature_values]
        ...
        for x in np.linspace(0, 1200, 20):  # Age
            ...
            for y in np.linspace(1, 30, 20):  # Hospitalization Duration
                row.append(model.predict(pd.DataFrame([[x, y, signs, temperature, respiratory_rate, time,
                                                        clinical_score, km, hazard_function,
                                                        drug_pencillin, drug_ceftriaxon, drug_ampicillin]],
                                                      columns=input_data.columns))[0])
        ...
        # Returning the text, gauge figure, progress bar value, scatter plot, and heatmap
